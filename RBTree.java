class RBTree
 {
     ...
     /* Constructor */
     public RBTree(int negInf,Object val)
     {
         header = new RedBlackNode(negInf,val);
     ...
     public void insert(int item,Object val )
     {
    ...
    /*public RedBlackNode search(int key) {
     ...
     {
         Search search1=search(header.right, val);
    	 return search1.tempNode;
     }
     private Search search(RedBlackNode r, int val)
     {
    	 Search search=new Search();
    	 search.found = false;
    	 search.tempNode=null;
         while ((r != nullNode) && !search.found)
         {
             int rval = r.element;
             if (val < rval)
                 r = r.left;
